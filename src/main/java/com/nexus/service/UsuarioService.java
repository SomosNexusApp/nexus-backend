...
public class UsuarioService implements UserDetailsService {
    ...
    public boolean verificarCuenta(String email, String codigo) {
        ...
        if (e == null || !e.cod().equals(codigo)) return false;
    ...
    public Map<String, Object> ingresarConGoogle(String tokenId) throws Exception {
        GoogleIdTokenVerifier v = new GoogleIdTokenVerifier.Builder(new NetHttpTransport(), GsonFactory.getDefaultInstance())
            .setAudience(List.of(googleClientId)).build();
            
        GoogleIdToken t = v.verify(tokenId);
        if (t == null) throw new IllegalArgumentException("Token inválido");
        ...
        if (existente.isPresent()) {
            ...
            nu.setEmail(email); 
            nu.setUser(usernameUnico(nombre));
    ...
    public String obtenerRol(Actor a) {
        if (a instanceof Admin) return "ADMIN"; 
        if (a instanceof Empresa) return "EMPRESA"; 
        return "USUARIO";
    }
